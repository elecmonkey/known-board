...
export default function MainView() {
  const { state, currentView, addTask, addTaskSet } = useApp();
  ...
  const filteredData = () => {
    ...
    const allTaskSets = state().taskSets;

    if (view === 'pending') {
      return {
        tasks: allTasks.filter(task => !task.completed && !task.parentId),
        taskSets: allTaskSets.filter(ts => !ts.hidden && !ts.parentId)
      };
      ...
      return {
        ...
        taskSets: allTaskSets.filter(ts => !ts.parentId)
      };
    }
  };
      ...
      <div class="space-y-4">
        ...
        {filteredData().tasks.length === 0 && filteredData().taskSets.length === 0 && (
          <div class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">暂无{getViewTitle()}</h3>
